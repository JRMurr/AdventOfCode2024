...
let
  ...
  getCompareFn =
    ...
    let
      ...
      compare =
        ...
        in
        # a is less than b
        if (builtins.elem b aLessThans) then
          true
        else
        # b is less than a
        if (builtins.elem a bLessThans) then
          false
        else
  ...
  part0 =
    ...
    let
      ...
      validUpdates = builtins.filter (
        updateLst: (isValidUpdate { inherit updateLst compareFn; }).isSorted
      ) updates;
  ...
  part1 =
    ...
    let
      ...
      sortedWithChecks = builtins.map (
        updateLst: (isValidUpdate { inherit updateLst compareFn; })
      ) updates;
